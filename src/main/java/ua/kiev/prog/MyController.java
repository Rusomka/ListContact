...
public class MyController {
    ...
    @GetMapping("/delete_group_page")
    public String groupDelete(Model model) {
        model.addAttribute("groups", contactService.findGroups());
        return "delete_group_page";
    }
    ...
    public String deleteGroup(@RequestParam(value = "group", required = false)
                              long[] delete) {
        if (delete != null && delete.length > 0 && delete[0] > 0) {
            contactService.deleteGroup(delete);
        }
        return "redirect:/";
    }
